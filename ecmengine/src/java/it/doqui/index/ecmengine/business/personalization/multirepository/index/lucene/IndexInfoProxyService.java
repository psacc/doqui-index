...
public class IndexInfoProxyService implements IndexInfoProxyServiceInterface {
	...
	public synchronized void init(DictionaryService dictionaryService, LuceneConfig luceneConfig, String repoPath) {
		...
		if(s_logger.isDebugEnabled()){
           s_logger.debug("[IndexInfoProxyService::init] (" +this +") Status: " + toStringStatic() + " - isSafe: " + isIntegritySafe());
           s_logger.debug("[IndexInfoProxyService::init] DST: " +this.dictionaryService  );
           s_logger.debug("[IndexInfoProxyService::init] DS: " +dictionaryService  );
           s_logger.debug("[IndexInfoProxyService::init] LC: " +luceneConfig       );
           s_logger.debug("[IndexInfoProxyService::init] RP: " +repoPath           );
        }
	}
	...
	private synchronized void initIndexInfo(String repoPath) {
		if(s_logger.isDebugEnabled()){
   		   s_logger.debug("[IndexInfoProxyService::initIndexInfo] (" +this +") IndexInfo : " +repoPath);
        }
	    ...
	    for(int lcv=0;lcv<stackElements.length;lcv++) {
            ...
            // Verifico se sono su una chiamata propagata da terracotta
	        if( !useTC && method.indexOf("_tc")!=-1 ){
	     	   useTC = true;
	        }

            // Log delle sole classi DOQUI e ALFRESCO
	     	if(s_logger.isDebugEnabled()){
    	       String className    = stackElements[lcv].getClassName();
	           if(className.indexOf("doqui")!=-1 || className.indexOf("alfresco")!=-1){
	     		   buffer.append( "ST_INDEXINFO["                       );
	     		   buffer.append( lcv                                   );
	     		   buffer.append( "]: "                                 );
        ...
        // Se sono su un metodo terracottato, stampa il log
	    if( useTC ){
	     	if(s_logger.isDebugEnabled()){
	            s_logger.debug("\n"+buffer+"\n");
	        }
		...
		if(s_logger.isDebugEnabled()){
		   s_logger.debug("[IndexInfoProxyService::initIndexInfo] (" +this +") IndexInfo pre : " + indexInfo);
        }

		indexInfo = IndexInfo.getIndexInfo(baseDir, config);

		if(s_logger.isDebugEnabled()){
		   s_logger.debug("[IndexInfoProxyService::initIndexInfo] (" +this +") IndexInfo post: " + indexInfo);
        }
	}
		...
		for (Document doc : docs) {
			try {
				insert(doc, writer);
      		    ...
      		    s_logger.error("[IndexInfoProxyService::insert] (" +this +") Failed to add document to index: " +e);
	...
	private synchronized void insert(Document doc, IndexWriter writer) throws LuceneIndexException, IOException {
		if(s_logger.isDebugEnabled()){
			s_logger.debug("[IndexInfoProxyService::insert] (" +this +") Writing doc: " + doc.getField("ID") + " [To directory: " + writer.getDirectory() + "]");
	...
	public synchronized void delete(Term term, String deltaId, String repoPath) throws LuceneIndexException, IOException {
    	if(s_logger.isDebugEnabled()){
    		s_logger.debug("[IndexInfoProxyService::delete] (" +this +") deltaId: " + deltaId);
	    }
	...
	public synchronized IndexReader getReader(String repoPath) throws LuceneIndexException, IOException {
		if(s_logger.isDebugEnabled()){
		   s_logger.debug("[IndexInfoProxyService::getReader] (" +this +") Status: " + toStringStatic() + " - isSafe: " + isIntegritySafe());
	    }
		initIndexInfo(repoPath);
	...
	public synchronized IndexReader getDeltaReader(String deltaId, String repoPath) throws LuceneIndexException, IOException {
		if(s_logger.isDebugEnabled()){
		   s_logger.debug("[IndexInfoProxyService::getDeltaReader] (" +this +") deltaId: " + deltaId);
	    }
		initIndexInfo(repoPath);
	...
	public synchronized void closeDeltaReader(String deltaId, String repoPath) throws IOException {
    	if(s_logger.isDebugEnabled()){
    		s_logger.debug("[IndexInfoProxyService::closeDeltaReader] (" +this +") deltaId: " + deltaId);
	    }
		initIndexInfo(repoPath);
	...
	private synchronized IndexWriter getDeltaWriter(String deltaId, String repoPath) throws LuceneIndexException, IOException {
    	if(s_logger.isDebugEnabled()){
    		s_logger.debug("[IndexInfoProxyService::getDeltaWriter] (" +this +") DS(" +(dictionaryService==null?"null":"object") +") deltaId: " + deltaId);
    	}
		initIndexInfo(repoPath);
	...
	public synchronized void closeDeltaWriter(String deltaId, String repoPath) throws IOException {
    	if(s_logger.isDebugEnabled()){
    		s_logger.debug("[IndexInfoProxyService::closeDeltaWriter] (" +this +") deltaId: " + deltaId);
	    }
		initIndexInfo(repoPath);
	...
	public synchronized void saveDelta(String deltaId, String repoPath) throws LuceneIndexException, IOException {
    	if(s_logger.isDebugEnabled()){
    		s_logger.debug("[IndexInfoProxyService::saveDelta] (" +this +") deltaId: " + deltaId);
    	}
	...
	public synchronized void closeMainReader(String repoPath) throws LuceneIndexException, IOException {
    	if(s_logger.isDebugEnabled()){
    		s_logger.debug("[IndexInfoProxyService::closeMainReader] (" +this +") Status: " + toString() + " - isSafe: " + isIntegritySafe());
    	}
	...
	public synchronized void setStatus(String deltaId, String repoPath, TransactionStatus state) throws IOException {
		if(s_logger.isDebugEnabled()){
		   s_logger.debug("[IndexInfoProxyService::setStatus] (" +this +") deltaId: " + deltaId + " - TransactionStatus: " + state);
	    }
		initIndexInfo(repoPath);
        ...
        if( ts!=null                         && // Ho il delta
    		...
    		s_logger.debug("[IndexInfoProxyService::setStatus] (" +this +") deltaId: " + deltaId + " - mantenuto TransactionStatus: " + state);
        } else {
    		s_logger.debug("[IndexInfoProxyService::setStatus] (" +this +") deltaId: " + deltaId + " - DA: " +ts +" A: " + state);
	...
	public synchronized void setPreparedState(String deltaId, String repoPath, Set<String> toDelete, long documents, boolean deleteNodesOnly) throws IOException {
    	if(s_logger.isDebugEnabled()){
    		s_logger.debug("[IndexInfoProxyService::setPreparedState] (" +this +") deltaId: " + deltaId);
	    }
		initIndexInfo(repoPath);
	...
	public synchronized IndexReader getMainIndexReferenceCountingReadOnlyIndexReader(String deltaId, String repoPath, Set<String> deletions, boolean deleteOnlyNodes) throws IOException {
    	if(s_logger.isDebugEnabled()){
    		s_logger.debug("[IndexInfoProxyService::getMainIndexReferenceCountingReadOnlyIndexReader] (" +this +") Deltaid: " +deltaId + " - repoPath: " +repoPath);
	    }
		initIndexInfo(repoPath);
	...
	public synchronized void setDictionaryService(DictionaryService dictionaryService) {
    	if(s_logger.isDebugEnabled()){
    		s_logger.debug("[IndexInfoProxyService::setDictionaryService] (" +this +") setDictionaryService: " +dictionaryService);
	    }
        ...
        if( dictionaryService!=null ){
           if( !(this.dictionaryService==dictionaryService) ){
               s_logger.error("[IndexInfoProxyService::setDictionaryService] (" +this +") setDictionaryService: " +this.dictionaryService);
               s_logger.error("[IndexInfoProxyService::setDictionaryService] (" +this +") setDictionaryService: " +dictionaryService);
           }
	    }
	}
