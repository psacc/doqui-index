...
import it.doqui.index.ecmengine.business.personalization.multirepository.index.lucene.RepositoryAwareADMLuceneIndexer;
import it.doqui.index.ecmengine.business.personalization.multirepository.index.lucene.RepositoryAwareAbstractLuceneIndexerImpl;
...
public class MultiRepositorySplittingADMIndexerImpl
...
implements RepositoryAwareADMLuceneIndexer {
	...
	/** Default constructor. */
	MultiRepositorySplittingADMIndexerImpl() {
		super();
	}
	...
	public void createNode(ChildAssociationRef relationshipRef) throws LuceneIndexException {
		...
		if (s_logger.isDebugEnabled()) {
			s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::createNode] " + "Repository '" + repository + "' -- Create node: " + relationshipRef.getChildRef());
		}
		...
		try {
			...
			if ((relationshipRef.getParentRef() == null) && tenantService.getBaseName(childRef).equals(nodeService.getRootNode(childRef.getStoreRef()))) {
				...
				s_logger.warn("[MultiRepositorySplittingADMIndexerImpl::createNode] " + "Repository '" + repository + "' -- " + "Detected root node addition: deleting all nodes from the index.");
			}
			index(childRef);
	...
	public void updateNode(NodeRef nodeRef) throws LuceneIndexException {
		...
		if (s_logger.isDebugEnabled()) {
			s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::updateNode] " + "Repository '" + repository + "' -- Update node: " + nodeRef);
		}
		checkAbleToDoWork(IndexUpdateStatus.SYNCRONOUS);
		try {
	...
	public void deleteNode(ChildAssociationRef relationshipRef) throws LuceneIndexException {
		...
		if (s_logger.isDebugEnabled()) {
			s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::deleteNode] " + "Repository '" + getRepository() + "' -- Delete node: " + relationshipRef.getChildRef());
		}
		checkAbleToDoWork(IndexUpdateStatus.SYNCRONOUS);
		try {
	...
	public void createChildRelationship(ChildAssociationRef relationshipRef) throws LuceneIndexException {
		...
		if (s_logger.isDebugEnabled()) {
			s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::createChildRelationship] " + "Repository '" + getRepository() + "' -- Create child: " + relationshipRef);
		}
		checkAbleToDoWork(IndexUpdateStatus.SYNCRONOUS);
		try {
	...
	public void updateChildRelationship(ChildAssociationRef relationshipBeforeRef, ChildAssociationRef relationshipAfterRef) throws LuceneIndexException {
		...
		if (s_logger.isDebugEnabled()) {
			s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::updateChildRelationship] " + "Repository '" + getRepository() + "' -- Update child '" + relationshipBeforeRef + "' to: "
	...
	public void deleteChildRelationship(ChildAssociationRef relationshipRef) throws LuceneIndexException {
		...
		if (s_logger.isDebugEnabled()) {
			s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::deleteChildRelationship] " + "Repository '" + getRepository() + "' -- Delete child: " + relationshipRef);
		}
		checkAbleToDoWork(IndexUpdateStatus.SYNCRONOUS);
		try {
			reindex(relationshipRef.getChildRef(), true);
	 ...
	 */
	public static MultiRepositorySplittingADMIndexerImpl getUpdateIndexer(StoreRef storeRef, String deltaId, String repository, LuceneConfig config) throws LuceneIndexException {
		if (s_logger.isDebugEnabled()) {
			s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::getUpdateIndexer] " + "Repository '" + repository + "' -- Creating indexer for store: " + storeRef + " [Delta: " + deltaId + "]");
		}
		MultiRepositorySplittingADMIndexerImpl indexer = new MultiRepositorySplittingADMIndexerImpl();
	...
	public List<Document> createDocuments(String stringNodeRef, boolean isNew, boolean indexAllProperties, boolean includeDirectoryDocuments) {
		...
		final Set<QName> aspects = (!isPart) ? nodeService.getAspects(nodeRef) : Collections.<QName> emptySet();
		final QName typeQName = (!isPart) ? nodeService.getType(nodeRef) : null;
		...
		// sia un nodo "parte"
		Map<QName, Serializable> properties = (!isPart) ? nodeService.getProperties(nodeRef) : Collections.<QName, Serializable> emptyMap();
		...
		if (dbId != null) {
			...
			if (s_logger.isDebugEnabled()) {
				s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::createDocuments] " + "Indexed DBID: " + dbId + " [NR: " + nodeRef + "]");
			}
		}
		...
		for (Pair<Path, QName> pair : paths) {
			...
			/*
			 * Per le parti e i nodi root la lista "paths" sara` sempre vuota, quindi non si entrera` mai in questo loop.
			 */
			...
			if (s_logger.isDebugEnabled()) {
				s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::createDocuments] Indexing PATH: " + pathString);
			}
		...
		if (isRoot) {
			...
			if (s_logger.isDebugEnabled()) {
				s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::createDocuments] Indexed ROOT: " + nodeRef + " [TX: " + nodeStatus.getChangeTxnId() + "]");
			}
			docs.add(xdoc);
			...
			if (s_logger.isDebugEnabled()) {
				s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::createDocuments] Indexed PART: " + nodeRef + " [TX: " + nodeStatus.getChangeTxnId() + "]");
			}
			docs.add(xdoc);
		} else {
			// not a root node
			...
			if (s_logger.isDebugEnabled()) {
				s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::createDocuments] Indexed NODE: " + nodeRef + " [TX: " + nodeStatus.getChangeTxnId() + "]");
			}
		...
		if (s_logger.isDebugEnabled()) {
			s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::createDocuments] Elapsed: " + (stop - start) + " ms");
		}
	...
	protected boolean indexProperty(NodeRef nodeRef, QName propertyName, Serializable value, Document doc, boolean indexAtomicPropertiesOnly, Serializable localeProperty){
		...
		for (Serializable serializableValue : DefaultTypeConverter.INSTANCE.getCollection(Serializable.class, value)) {
			...
			if (isContent) {
				...
				if (locale == null) {
					//                    Serializable localeProperty = nodeService.getProperty(nodeRef, ContentModel.PROP_LOCALE); // XXX: accesso evitabile
				...
				ContentReader reader = contentService.getReader(nodeRef, propertyName);  // fornisce un reader per il nodo aggiunto
				// contentService è un oggetto di tipo CryptoContentServiceImpl
	...
	private boolean mayHaveChildren(QName typeQName, Set<QName> aspects) {
		...
		TypeDefinition nodeTypeDef = getDictionaryService().getType(typeQName);
	...
	private ArrayList<NodeRef> getParents(Path path) {
		ArrayList<NodeRef> parentsInDepthOrderStartingWithSelf = new ArrayList<NodeRef>(12);
	...
	//  private Collection<Pair<Path, QName>> getCategoryPaths(NodeRef nodeRef, Map<QName, Serializable> properties)
	{
		...
		//      Set<QName> aspects = nodeService.getAspects(nodeRef);   // XXX: evita accessi ripetuti NS
	...
	public int updateFullTextSearch(int size) throws LuceneIndexException {
		...
		// if (!mainIndexExists())
			...
			// }
		try {
			// Settiamo il repository con l'ID associato a questo indexer
			NodeRef lastId = null;
			...
			if (!toFTSIndex.isEmpty()) {
				...
				if (idx != -1) {
					...
					s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::updateFullTextSearch] found node on tenant store [" + tenantDomain + "]");
					String adminUser = AuthenticationUtil.getSystemUserName() + TenantService.SEPARATOR + tenantDomain;

					s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::updateFullTextSearch] changing user to tenant admin user: " + adminUser);
				...
				try {
					...
					//                    writer = service.getDeltaWriter();

					ArrayList<Helper> failedNodes = new ArrayList<Helper>();
					...
					for (Helper helper : toFTSIndex) {
						...
						try {
							docs = createDocuments(ref.toString(), false, true, false);
						} catch (InvalidNodeRefException e) {

							s_logger.warn("[MultiRepositorySplittingADMIndexerImpl::updateFullTextSearch] " + "Repository '" + RepositoryManager.getCurrentRepository()
									+ "' -- Docs generation failed for node: " + ref, e);

							failedNodes.add(helper);
							continue;
						}
                        ...
                        if (s_logger.isDebugEnabled()) {
						    s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::updateFullTextSearch] " + "Repository '" + RepositoryManager.getCurrentRepository() + "' -- Generated " + docs.size()
								+ " doc(s) for node: " + ref);
                        }
                    ...
                    if (s_logger.isDebugEnabled()) {
    					s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::updateFullTextSearch] " + "Repository '" + RepositoryManager.getCurrentRepository() + "' -- Generated " + totalDocs
							+ " doc(s) total.");
                    }

					// Remove failed nodes
					toFTSIndex.removeAll(failedNodes);

					int done = (int) service.getDocCount(deltaId, getRepoStorePath());
					remainingCount = count - done;

                    if (s_logger.isDebugEnabled()) {
    					s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::updateFullTextSearch] " + "Repository '" + RepositoryManager.getCurrentRepository() + "' -- " + remainingCount
					...
					s_logger.error("[MultiRepositorySplittingADMIndexerImpl::updateFullTextSearch] " + "Repository '" + RepositoryManager.getCurrentRepository() + "' -- ERROR IN FTS INDEX!");
			...
			//          setRollbackOnly();
			s_logger.error("[MultiRepositorySplittingADMIndexerImpl::updateFullTextSearch] " + "FTS update fallito sul repository: " + getRepository() + " - Uno o piu` nodi non trovati.");
			throw new LuceneIndexException("Failed FTS update on repository: " + getRepository());
		}
	}
	...
	protected void doCommit() throws IOException {
		if (indexUpdateStatus == IndexUpdateStatus.ASYNCHRONOUS) {
			...
			s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::doCommit] Requesting FTS indexing of: " + store + " - " + getRepository());
			fullTextSearchIndexer.requiresIndex(store, getRepository());
		}
		if (callBack != null) {
			s_logger.debug("[MultiRepositorySplittingADMIndexerImpl::doCommit] " + "Repository '" + getRepository() + "' -- " + "Indexing completed: " + store + " - " + getRepository());
